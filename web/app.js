...
<div>
<header>
  <h1>Laya &mdash; <span x-text="modelName || 'local model'"></span> plays MARS RAID</h1>
...
<main>
  ...
  <aside>
    ...
    situation(full = true) {
      ...
      /* Saucers only reach the ballot once they are actually doing damage -- the gliner-mars finding,
         and it matters more here. Laya scores options by overlap with the state, so an option whose
         word appears in the text wins regardless of what the sentence says about it. Gating the
         option is the robust half of the fix; not naming absent things (see server/laya_server.py's
         describe()) is the other half. */
      ...
      Object.assign(questions, {
        /* No posture question. Asked to choose between attacking and escaping, Laya answers at
           0.01-0.15 confidence and does not track danger -- the same failure GLiNER had. Its `threat`
