...
const MODEL = process.env.MODEL_URL ?? 'http://127.0.0.1:8780';
const PORT = Number(process.env.PORT ?? 8734);

Bun.serve({
  port: PORT, hostname: '127.0.0.1',
  // A decision is a full diffusion generation on a 26B model, several seconds; 10 s would abort it.
  idleTimeout: 120,
  async fetch(req) {
    const url = new URL(req.url);
    if (url.pathname === '/api/decide' && req.method === 'POST') {
      const r = await fetch(`${MODEL}/v1/decide`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: await req.text() });
      return new Response(await r.text(), { status: r.status, headers: { 'Content-Type': 'application/json' } });
    }
    if (url.pathname === '/api/health') {
      // The decision server reports {ok, backend, model}; normalise so the page does not care.
      try {
